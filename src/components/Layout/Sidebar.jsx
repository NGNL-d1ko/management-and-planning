      ...
      <div className="ym-sidebar-brand d-flex align-items-center justify-content-between gap-2 px-3 py-4 border-bottom">
        ...
        {onMinimizeToggle && (
          <Button
            ...
            aria-label={isMinimized ? t('layout.expandSidebar') : t('layout.collapseSidebar')}
            title={isMinimized ? t('layout.expand') : t('layout.collapse')}
          >
      ...
      <Nav className="ym-sidebar-nav flex-column gap-1 p-3">
        {navigationItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon size={18} />
              <span className="ym-sidebar-label">{label}</span>
            </Nav.Link>
          );
        })}
      </Nav>
